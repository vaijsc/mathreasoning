...
class CustomEmbedding(Module):
    ...
    def __init__(self, original_embeddings, token_ids: List, finetune_new_vocab):
        super(CustomEmbedding, self).__init__()
        self.original_embeddings = original_embeddings
        self.token_ids = token_ids
        self.extra_token_embeddings = ParameterList([Parameter(data=original_embeddings.weight[token_id].clone(), requires_grad=finetune_new_vocab) for token_id in self.token_ids])
...
def extend_vocab(model, tokenizer, finetune_new_vocab=False, num_new_tokens=100):
    """
