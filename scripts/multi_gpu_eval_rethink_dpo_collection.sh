...
checkpoint_name=$5
ul2_rethink=$6
model_path="saves/$model_name/$checkpoint_name"
dependency=$7
...
#SBATCH --job-name=gen_{{model_name}}_{{checkpoint_name}}_dpo_cot_rethink_{{cur_partition}}
...
python LLaMA-Factory/src/utils/infer_ul2.py \
    ...
    --out_file infer_res/dpo_collect_cot/{{model_name}}_{{checkpoint_name}}_{{output_dataset_prefix}}_dpo_cot_rethink_bartmixed0616_{{cur_partition}}.json \
    --batch_size 1 \
    --ul2 \
    --sc cot \
    --cot_mode "greedy" \
...
for ((cur_partition=0; cur_partition<num_gpu; cur_partition++)); do
  ...
  echo "$job_script_filled" > "scripts/dpo_collect_${cur_partition}.sh"
  sbatch scripts/dpo_collect_${cur_partition}.sh
done
