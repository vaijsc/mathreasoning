...
#SBATCH --job-name=gen_{{model_name}}_{{checkpoint_name}}_cot_rethink_bartmixed0616_{{cur_partition}}
...
python LLaMA-Factory/src/utils/infer_ul2.py \
    ...
    --out_file infer_res/{{model_name}}_{{checkpoint_name}}_{{output_dataset_prefix}}_cot_rethink_bartmixed0616_review_scratch_{{cur_partition}}.json \
    --batch_size 1 \
    --ul2 \
    --sc cot \
    --cot_mode "greedy" \
