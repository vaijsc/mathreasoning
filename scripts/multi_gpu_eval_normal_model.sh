...
#SBATCH --job-name=gen_{{model_name}}_{{checkpoint_name}}_{{cur_partition}}_norethink_cot
...
python LLaMA-Factory/src/utils/infer_ul2.py \
    --base_path /home/hieupq1/hieupq1/math/saves/{{model_name}}/{{checkpoint_name}} \
    --dataset_path cache/{{dataset_split}} \
    --out_file infer_res/sft_base_cot/{{model_name}}_{{checkpoint_name}}_{{output_dataset_prefix}}_{{cur_partition}}_norethink_cot.json \
    --causal_prefix \
    --batch_size 1 \
    --sc cot
'
