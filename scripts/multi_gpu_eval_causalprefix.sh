...
#SBATCH --job-name=gen_{{model_name}}_{{checkpoint_name}}_rethink_bartmixed_{{cur_partition}}
...
python LLaMA-Factory/src/utils/infer_ul2.py \
    ...
    --out_file infer_res/{{model_name}}_{{checkpoint_name}}_{{output_dataset_prefix}}_{{cur_partition}}_norethink.json \
    --batch_size 1 \
    --ul2 \
    --sc cot \
    --cot_mode "greedy" \
